						...
						_update=function(me)
							...
							if inrange(me_x,me_y) then
								if me.st<99 then
									...
									if #me.smoke>0 then
										foreach(me.smoke,function(s) s:_update() end)
									end
								end
...
function add_light(x,y,sst)
	...
	create_tank(x,y,{
		st=sst,sprh=108,sprv=109,spr_w=1,spr_h=1,off=4,wait=4,hp=35,px=8,
		spd=.35,mode=2,
		hb={x=-4,y=-4,w=8,h=8},
		radar={x=-32,y=-32,w=64,h=64},
...
function add_heavy(x,y,sst)
	...
	create_tank(x,y,{
		st=sst,sprh=110,sprv=124,spr_w=2,spr_h=2,off=8,wait=5,hp=65,px=11,
		spd=.2,mode=3,
		hb={x=-5,y=-5,w=10,h=10},
		radar={x=-45,y=-45,w=90,h=90},
		attack={qty=5,pow=1,between=0,aiminc=.03125}
	})
end
...
function add_heli()
	local obj={
		heli=true,
		st=0,timer=objtimer,life=t(),hp=40,dmg=false,dmgt=0,f=0,laser=false,
	...
	local obj={
		...
		_update=function(me)
			if me.t==0 then
				...
				me.aim=atan2(tx-me.x, ty-me.y)
				
				if me.between==0 and me.aiminc then
					local h=flr(me.qty/2)
					for n=1,h do me.aim-=me.aiminc end
				end
...
function snake_props(attackType)
	...
	local obj={
		...
		_update=function(me)
			...
			me.nearest=false
			local nearest_d=999
			local gun_range=55 --default range for bullets/spread
			
			if me.at==3 then gun_range=80 end --canon
			if me.at==4 then gun_range=50 end --laser
	...
	if attackType==3 then obj.br=rnd(3)+2 end --canon bullet rate
	if attackType==5 then obj.br=rnd(2)+1 end --spread bullet rate
...
scene_play={
	...
	_draw=function()
		...
		text_shadow('fps:'..stat(7), camx+1, camy+116, 7)
